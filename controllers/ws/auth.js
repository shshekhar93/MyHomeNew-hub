  ...
  const device = devices.find(({encryptionKey}) => {
    try {
      ...
      [
        deviceName,
        sessionKey
      ] = decryptedPayload.split('|').map(s => s.trim());
...
async function authenticateNewDevice(username, authStr) {
  ...
  const {
    device, deviceName, sessionKey
  } = decrypt(authStr, allDevices);
...
async function authenticateDevice(username, authStr) {
  try {
    ...
    let result = await authenticateConfiguredDevice(username, authStr);
    if(result) {
      return result;
    }
