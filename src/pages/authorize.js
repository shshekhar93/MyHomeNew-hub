...
function AuthorizePage() {
  ...
  return (
    <div className={css({
      ...
      <p>Once authorized, {client.name} will be able to discover and control the devices that you add to your Home Applyed account.</p>
      <form action="/authorize" method="POST">
        <input type="hidden" name="grant_type" value="code" />
        {Object.entries(params).map(([name, value]) => 
          <input key={name} type="hidden" name={name} value={value} />
        )}
        <Button type="submit" $size="expand">Authorize</Button>
      </form>
    </div>
  );
}
